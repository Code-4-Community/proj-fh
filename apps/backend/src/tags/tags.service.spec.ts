...
describe('TagsService', () => {
    ...
    const createTagDto = { category: Category.FOOD_SERVICE_TYPE, label: 'Farmers Market', slug: 'farmers-market' };
    ...
    it('creates a tag', async () => {
		...
		await expect(service.create(createTagDto.category, createTagDto.label, createTagDto.slug)).resolves.toEqual(
			createdTag,
		);
	...
	it('propagates errors from the database', async () => {
		...
		await expect(
			service.create(createTagDto.category, createTagDto.label, createTagDto.slug),
		).rejects.toBe(error);
	});

	it('rejects when category is missing', async () => {
		// @ts-ignore Intentionally passing a missing category to test validation.
		await expect(service.create(undefined, createTagDto.label, createTagDto.slug)).rejects.toThrow(
			new BadRequestException('Category, label, and slug are required to create a tag.'),
		);
	...
	it('rejects when label is missing', async () => {
		...
		await expect(service.create(createTagDto.category, undefined, createTagDto.slug)).rejects.toThrow(
			new BadRequestException('Category, label, and slug are required to create a tag.'),
		);
		expect(repository.save).not.toHaveBeenCalled();
	});
	...
	it('rejects when slug is missing', async () => {
		...
		await expect(service.create(createTagDto.category, createTagDto.label, undefined)).rejects.toThrow(
			new BadRequestException('Category, label, and slug are required to create a tag.'),
		);
		expect(repository.save).not.toHaveBeenCalled();
	});
	...
	it('rejects a blank label', async () => {
		await expect(service.create(createTagDto.category, '   ', createTagDto.slug)).rejects.toThrow(
			new BadRequestException('Label cannot be empty.'),
		);
		expect(repository.save).not.toHaveBeenCalled();
	});

	it('rejects a blank slug', async () => {
		await expect(service.create(createTagDto.category, createTagDto.label, '   ')).rejects.toThrow(
